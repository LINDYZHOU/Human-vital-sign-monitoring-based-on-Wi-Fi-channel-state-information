%% read the CSI file
csi1 = CSIdata_in (ax40_regular60s_zxl,'hesu','Mag');
...
%     csi_filter=filter(lowpass,frame);
    ...
    [csi_selected_h,selection_h{n}] = subcarrier_selection_h(rate,frame,10);
    ...
    emd_csi_h = emd_hb(csi_selected_b,1);
    emd_csi_h2 = emd_hb(csi_selected_h,1);
...
A_breath_rate_pp=mean(a_breath_rate_pp_fd);
% fprintf('PEAK GAP: Respiratory rate: %fHz. \n', A_breath_rate_pp);
A_breath_rate_dft = mean(a_breath_rate_dft_fd);
fprintf('Respiratory rate: DFT:  %fHz.   PEAK GAP: %fHz.\n', A_breath_rate_dft, A_breath_rate_pp);

A_heart_rate_pp=mean(a_heart_rate_pp_fd);
% fprintf('PEAK GAP: Heart rate: %fHz. \n', A_heart_rate_pp);
A_heart_rate_dft = mean(a_heart_rate_dft_fd);
fprintf('Heart rate: DFT: %fHz.     PEAK GAP: %fHz.\n', A_heart_rate_dft, A_heart_rate_pp);

A_heart_rate_pp2=mean(a_heart_rate_pp_fd2);
A_heart_rate_dft2 = mean(a_heart_rate_dft_fd2);
