...
[num_s,~]=size(csi);
%number of sample, number of subcarriers
num_f=round((num_s-frame_length*rate)/(rate*frame_interval));
%number of frame
i=1;
sequence=sequence+1;
% the index is from 1 to 4096
[~,locs] = findpeaks(sequence);
% find the start and end of the circulation
locs = [0;locs;length(sequence)]';
%the first circulation is beginning from 0
num_matrix = length(locs)-1;
% the number of circulation.
sequence_index=[];

%for every circulation
for num = 1:num_matrix
    matrix=sequence((locs(num)+1):locs(num+1));
    %sequence matrix every 4096
    index_matrix=4096*(num-1)+matrix-sequence(1)+1;
    % the index of them consecutive
